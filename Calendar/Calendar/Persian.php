 ...
 */
class Persian extends AbstractCalendar
{
    ...
    {
        $timestamp = $this->datetime->getTimestamp() + $this->datetime->getOffset();
        ...
        $this->calendarTime->setSecond($seconds);
        $this->calendarTime->setMinute($minutes);
        $this->calendarTime->setHour($hours);
        $this->calendarTime->setDay($days);
        $this->calendarTime->setYear($years);
        $this->calendarTime->setMonth($months);
        $this->calendarTime->setDayOfYear($dayOfYear);
    ...
    {
        $timestamp = $this->calendarTime->getSecond();
        $timestamp += $this->calendarTime->getMinute() * 60;
    ...
    {
        // Day
        $format = str_replace("d", str_pad($this->calendarTime->getDay(), 2, '0', STR_PAD_LEFT), $format);
        ...
        $format = str_replace("j", $this->calendarTime->getDay(), $format);
        ...
        $format = str_replace("z", $this->calendarTime->getDayOfYear() - 1, $format);
        ...
        $format = str_replace("m", str_pad($this->calendarTime->getMonth(), 2, '0', STR_PAD_LEFT), $format);
        $format = str_replace("M", $this->getMonthName("short"), $format);
        $format = str_replace("n", $this->calendarTime->getMonth(), $format);
        $format = str_replace("t", "number of days in given month", $format);  //TODO: fix it

        // Year
        $format = str_replace("L", (int) $this->isLeap(), $format);
        $format = str_replace("o", $this->calendarTime->getYear(), $format);
        $format = str_replace("Y", $this->calendarTime->getYear(), $format);
        $format = str_replace("y", $this->calendarTime->getYear() % 100, $format);

        // Time
        $format = str_replace(array('a', 'A'), $this->calendarTime->getHour() < 12 ? self::$words['AM'] : self::$words['PM'], $format);
        $format = str_replace("B", 000, $format);
        $format = str_replace("g", $this->calendarTime->getHour() % 12, $format);
        $format = str_replace("G", $this->calendarTime->getHour(), $format);
        $format = str_replace("h", str_pad($this->calendarTime->getHour() % 12, 2, '0', STR_PAD_LEFT), $format);
        $format = str_replace("H", str_pad($this->calendarTime->getHour(), 2, '0', STR_PAD_LEFT), $format);
        $format = str_replace("i", str_pad($this->calendarTime->getMinute(), 2, '0', STR_PAD_LEFT), $format);
        $format = str_replace("s", str_pad($this->calendarTime->getSecond(), 2, '0', STR_PAD_LEFT), $format);
    ...
    {
        $x = (7 - $this->dayOfWeek($this->calendarTime->getYear(), 1) ) % 7;
        $z = $this->calendarTime->getDayOfYear() - $x;
        return abs(ceil($z / 7));
    }

    private function dayOfWeek()
    {
        $dayOfYear = $this->calendarTime->getDayOfYear();
        $yearValue = $this->calendarTime->getYear();
    ...
    private function isLeap() {
        $rasad = $this->calendarTime->getYear() + 2346;
